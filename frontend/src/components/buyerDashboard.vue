...
let storedOULResults = {}; // Define storedOULResults in a scope accessible to testData
const loading = ref(true);
const inventoryDataLoaded = ref(false);
const inventoryData = ref({}); 
const oulValue = ref(0);
const ssValue = ref(0);
const chartData = ref([]); 


main(); // Call the main function
fetchItemsInInventory("qpNHH54FfF9k1vXv8gNV");

// Reactive computed properties for filtered items based on selected category
...
function onCategoryChange() {
  selectedItem.value = null; // Reset selected item when category changes
...
function updateChartData(item) {
      if (item) {
        ...
        oulValue.value = getOUL(item); // Fetch OUL for the selected item
        ssValue.value = getSS(item);   // Fetch Safety Stock for the selected item

    chartData.value = [
        { name: '2023-01', Total: 20, OUL: oulValue.value, SafetyStock: ssValue.value },
        { name: '2023-02', Total: 30, OUL: oulValue.value, SafetyStock: ssValue.value },
        { name: '2023-03', Total: 10, OUL: oulValue.value, SafetyStock: ssValue.value },
    ];
}
}
...
export default {
  data() {
    return {
      chartData: [], // Initialize chartData
...
async created() {
    try {
      await main(); // Ensure main() is called during component creation
      
      // Check if there are items after initial fetch
      const items = Object.keys(filteredItems.value);
      if (items.length > 0) {
        selectedItem.value = items[0]; // Auto-select the first item
      }

      // Initialize chart data for the selected item
      updateChartData();
...
async function fetchItemsInInventory(restaurantId) {
    const restaurantCollection = doc(db, 'restaurant', restaurantId); // Use doc() to reference a specific document
    const restaurantSnapshot = await getDoc(restaurantCollection); // Use getDoc() to retrieve the document

    if (!restaurantSnapshot.exists()) { // Use exists() method
    ...
    // Assuming inventoryData is defined as a ref elsewhere in your code
    inventoryData.value = restaurantData['Inventory Level'] || {}; // Use data from the restaurant document
    console.log("Fetched Inventory Data:", inventoryData.value); // Debugging output
    inventoryDataLoaded.value = true; // Mark data as loaded
    loading.value = false; // Update loading state, ensure 'loading' is defined as a ref

    // Auto-select the first category
    const categories = Object.keys(inventoryData.value);
    if (categories.length > 0) {
      selectedCategory.value = categories[0]; // Auto-select first category
      onCategoryChange(); // Call the change handler to set selectedItem
    }
}
...
// call function to check inventory items
...
async function calculateMeanDemandAndSd(restaurantId) {
    ...
    const categoryMonthlySums = {}; // To store the total quantity per month for each category
    ...
    for (const [month, categories] of Object.entries(actualDemand)) {
        ...
        for (const [category, items] of Object.entries(categories)) {
            ...
            const monthTotal = Object.values(items).reduce((sum, quantity) => sum + quantity, 0); // Sum quantities for the month

            // Initialize if not already done
            if (!categoryMonthlySums[category]) {
                categoryMonthlySums[category] = []; // Array to hold monthly totals
            }

            // Store the monthly total for this category
            categoryMonthlySums[category].push(monthTotal);
        }

        // Iterate through each item in the month
        for (const [category, items] of Object.entries(categories)) {
...
function calculateZScore(serviceLevel) {
    if (serviceLevel < 0 || serviceLevel > 100) {
        serviceLevelWarning.value = "Please enter a value between 0 and 100.";
    }
    // Using the inverse of the standard normal cumulative distribution function
    else {
        serviceLevelWarning.value = "";
    }
    return jStat.normal.inv(serviceLevel/100, 0, 1);
}

// Example usage
// const serviceLevel = 96; // this will be by user input in the future
...
async function main() {
    ...
    if (results) {
        ...
        for (const [item, params] of Object.entries(storedOULResults)) {
    ...
    // console.log(storedOULResults['Beef']['OUL']);
}
